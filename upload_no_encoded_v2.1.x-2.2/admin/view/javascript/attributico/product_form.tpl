      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          <ul class="nav nav-tabs">
           ...
           <li><a href="#tab-variant" data-toggle="tab"><?php echo $tab_variant; ?></a></li>
			...
			*/ ?>
            ...
            <div class="tab-pane" id="tab-attribute">
              <div class="table-responsive">
                ...
                <td class="text-left"><?php echo $entry_attribute; ?>
                    <button type='button' id='attach-attribute' class='btn btn-success pull-right'><i class='fa fa-plus'></i> <?php echo $tab_attribute; ?></button>
                </td>
                ...
                <td class="text-left">
                  ...
                  <tbody>
                    ...
                    <tr id="attribute-row<?php echo $attribute_row; ?>">
                      <td class="text-left" style="width: 40%;"><label id="group-name<?php echo $attribute_row; ?>"><?php echo $product_attribute['group_name']; ?></label>                        
...
<td class="text-left" style="width: 5%;"><input type="text" value="<?php echo $product_attribute['sort_order']; ?>" class="form-control" readonly="true"/></td>
            ...
            <select  onclick="selectvalue(<?php echo $attribute_row; ?>,<?php echo $language['language_id']; ?>)" name="attr_val_id[<?php echo $attribute_row; ?>][<?php echo $language['language_id']; ?>]" class="form-control">
              <option value="" selected="selected">----</option>
                <?php foreach($product_attribute['product_attribute_all_value'] as $valueall) {
                  if($valueall['langid'] === $language['language_id']) { ?>
                    <option value="<?php echo $valueall['text']; ?>"><?php echo $valueall['text']; ?></option>
                  <?php } } ?>
            </select>
            
                        </div>
                        <?php } ?></td>
                      <td class="text-left"><button type="button" onclick="product_attribute_id = product_attribute_id.filter(item=>item !== $('#attribute-row<?php echo $attribute_row; ?>').find('[name *= attribute_id]').val()); $('#attribute-row<?php echo $attribute_row; ?>').remove(); " data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>
            ...
            <?php } else { ?>
			 <div class="form-group"><p style="font-size: 22px;text-align: center;">Модуль не установлен!</p></div>			 
			<?php } ?>
            <?php if (isset($sizes)) { ?>			
			<div class="form-group">
                <label class="col-sm-2 control-label" for="input-related-variant"><?php echo $entry_size_table; ?></label>
                <div class="col-sm-10">
...
function addAttribute(attribute_id='""') {
	...
	html += '  <td class="text-left" style="width: 20%;"><input type="text" name="product_attribute[' + attribute_row + '][name]" value="" placeholder="<?php echo $entry_attribute; ?>" class="form-control" /><input type="hidden" name="product_attribute[' + attribute_row + '][attribute_id]" value=' + attribute_id  + ' /></td>';
	...
	<?php foreach ($languages as $language) { ?>
	html += '<div class="input-group"><span class="input-group-addon"><img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /></span><textarea name="product_attribute[' + attribute_row + '][product_attribute_description][<?php echo $language['language_id']; ?>][text]" rows="5" placeholder="<?php echo $entry_text; ?>" class="form-control"></textarea><select  onclick="selectvalue(' + attribute_row + ',<?php echo $language['language_id']; ?>);" name="attr_val_id[' + attribute_row + '][<?php echo $language['language_id']; ?>]" class="form-control"><option value="" selected="selected">----</option></select></div>';
    <?php } ?>
	html += '  </td>';
	html += '  <td class="text-left"><button type="button" onclick="product_attribute_id = product_attribute_id.filter(item=>item !== $(\'#attribute-row' + attribute_row + '\').find(\'[name *= attribute_id]\').val()); $(\'#attribute-row' + attribute_row + '\').remove(); " data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>'
...
selectAttributeAutocomplete(attribute_row, old_id, item)
<?php foreach ($languages as $language) { ?>
            html<?php echo $language['language_id']; ?> = '<option value="">----</option>';
            jQuery.each(item.values, function() {
              var textvalue = this.text;
              var langid = this.langid;
              if(langid == <?php echo $language['language_id']; ?>){
                html<?php echo $language['language_id']; ?> += '  <option value="' + textvalue + '">' + textvalue + '</option>';
              }
            });
            $('select[name=\'attr_val_id[' + attribute_row + '][<?php echo $language['language_id']; ?>]\']').html(html<?php echo $language['language_id']; ?>);
            <?php } ?>
...
function selectvalue(attribute_row,langid) {
                 var datavalueattr = $('select[name=\'attr_val_id[' + attribute_row + '][' + langid + ']\'] :selected').val();
                  if(datavalueattr != '')
                    {
                      $('textarea[name=\'product_attribute[' + attribute_row+ '][product_attribute_description][' + langid + '][text]\']').val(datavalueattr);
                    }
                }
...
<script src="view/javascript/attributico/attributico-product.js" type="text/javascript"></script>
...
<script>
  $(document).ready(function() {
    ...
    if($('tr[id^="image-row"').length){
            var image_rows = parseInt($('#images tbody>tr:last-child').attr('id').replace(/\D+/g,""));
        } else {
            var image_rows = 0;
        }
      ...
      var k=1;
      var n=1;
      if(images[1] && !$('tr[id^="image-row"').length){
        addImage();
      }
        } else {
      var k=0;
      var n=0;
      if(images[0] && !$('tr[id^="image-row"').length){
        addImage();
      }
    }
    
    if(image_rows==0){
      image_rows = parseInt($('#images tbody>tr:last-child').attr('id').replace(/\D+/g,""));
      for(var i = image_rows; i < image_rows+images.length-k; i++) {
              if(!$('#input-image' + i).length) { addImage(); }
              $('#input-image' + i).val(images[n]);
              $('#thumb-image' + i + ' img').attr('src', '../image/' + images[n]).css('max-width', '100px');
              n++;
          }
    } else {
          for(var i = image_rows; i < image_rows+images.length-k; i++) {
  ...
  $(document).ready(function() {
    ...
    if($('tr[id^="image-row"').length){
        ...
        if(images.length === 1) {
            if(!$('#input-image' + image_rows).length) { addImage(); }
      image_rows = parseInt($('#images tbody>tr:last-child').attr('id').replace(/\D+/g,""));
          $('#input-image'+image_rows).val(images[0]);
          $('#thumb-image' + image_rows + ' img').attr('src', '../image/' + images[0]).css('max-width', '100px');

        } else {
      if(!$('tr[id^="image-row"').length || ($('tr[id^="image-row"').length && $('#input-image' + image_rows).val()!='')) { addImage(); }
      image_rows = parseInt($('#images tbody>tr:last-child').attr('id').replace(/\D+/g,""));
          for(var i = 0; i <= images.length-1; i++) {
            if(!$('#input-image' + (image_rows + i)).length && $('#input-image' + (image_rows + i)).val()!='') { addImage(); }
            $('#input-image' + (image_rows + i)).val(images[i]);
            $('#thumb-image' + (image_rows + i) + ' img').attr('src', '../image/' + images[i]).css('max-width', '100px');
          }          
        }

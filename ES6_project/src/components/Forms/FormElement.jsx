...
import Autocomplete from './FormComponent/Autocomplete';
import AsyncCreateSelect from './FormComponent/AsyncCreateSelect';

// Reusable Form Component
function FormInput({ register, element, errors, idForm, data }) {
    ...
    console.log('render Register', fieldName);
    console.log('get Register', register); */
    switch (type) {
        case 'email':
            return (
                <div>
                ...
                <div>
                    <div className={'form-group' + (errors[name] ? ' has-error' : '')}>
                        ...
                        <div className="col-sm-10">
                            <Autocomplete
                                name={name} defaultValue={value} placeholder={placeholder ? placeholder : label} data={data} ref={register(validationProps)}
                            />
                            {errors[name] && <div className="text-danger">{errors[name]['message']}</div>}
                        </div>
                    </div>
            ...
            return (
                <div>
                    <div className={'form-group' + (errors[name] ? ' has-error' : '')}>
                        ...
                        <div className="col-sm-10">
                            <AsyncCreateSelect
                                name={name} form={idForm} defaultValue={value} placeholder={placeholder ? placeholder : label} inputRef={register(validationProps)}
                            />
                            {errors[name] && <div className="text-danger">{errors[name]['message']}</div>}
                        </div>
                    </div>
        ...
        case 'hidden':
            return (
                <div>
                    <input type="hidden" className="form-control" name={name} id={name} defaultValue={value} ref={register(validationProps)} />
                </div>
            );
        case 'input-group':
            return (

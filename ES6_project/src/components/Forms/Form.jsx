...
function Form({ template, data, onSubmit, onCancel, watchFields, validate, idForm }) {
    ...
    const renderElements = (elements) => {
        return elements.map(element => {
            if ('cols' in element) {
                        ...
                        {element.cols.map(col => {
                            return (
                                <div key={col.name} className={"col-sm-" + col.width + " col-md-" + col.width + " col-xs-12"}>
                                    <FormInput key={element.name} register={register} element={col} errors={errors} data={data}></FormInput>
                                </div>
                            )
                        })}
                    </div>
                ...
                return (
                    <FormInput key={element.name} register={register} element={element} errors={errors} data={data}></FormInput>
                )
            }
        })
    }
